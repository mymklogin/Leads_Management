...
public class Template1DtmfStrategy : ITemplateWebhookStrategy
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch (eventType)
        {
            ...
            case "DTMF":
                if (pressedKey == "1")
                {
                    leadStatus = "Interested Lead - Pressed 1";
                }
                else if (!string.IsNullOrWhiteSpace(pressedKey))
                {
                    leadStatus = $"Interactive Lead - Other Key Pressed ({pressedKey})";
                }
                else
                {
            ...
            case "HANGUP":
                ...
                {
                    leadStatus = "Warm Lead - Long Call Without Target Key";
                }
                else if (duration >= 5)
                {
                    leadStatus = "Cold Lead - Short Interaction";
                }
                else
                {
                    leadStatus = "Dropped / Ignored";
                }
                break;
